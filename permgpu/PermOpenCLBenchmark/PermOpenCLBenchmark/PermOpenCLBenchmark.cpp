...
void CheckPermuteWithOpenCL(char* arrDest, long long Max);
void CheckNextPermuteWithOpenCL(char* arrDest, long long Max, int next_permutation_iteration);
...
void Cleanup(cl_context context, cl_command_queue commandQueue,
	cl_program program, cl_kernel kernel, cl_kernel kernel2, cl_mem memObjects[2])
{
	for (int i = 0; i < 2; i++)
	{
		if (memObjects[i] != 0)
...
int main(int argc, char** argv)
{
	...
	if (commandQueue == NULL)
	{
		Cleanup(context, commandQueue, program, kernel, kernel2, memObjects);
		return 1;
	}

	// Create OpenCL program from HelloWorld.cl kernel source
	char file[] = "C:\\Users\\wong\\Documents\\Visual Studio 2010\\Projects\\PermOpenCLBenchmark\\PermOpenCLBenchmark\\PermOpenCLBenchmark.cl";
	...
	kernel2 = clCreateKernel(program, "run_next_permutation", NULL);
	if (kernel2 == NULL)
	{
	...
	if (!CreateMemObjects(context, memObjects, arrDest, &Max))
	{
		Cleanup(context, commandQueue, program, kernel, kernel2, memObjects);
		return 1;
	}
	...
	CheckPermuteWithOpenCL(arrDest, Max);
	...
	for(int i=0; i<NEXT_PERM_LOOP; ++i)
	{
		...
		sprintf(buf, "next_permutation Timing: %dms\n", timing);
		std::cout<<buf<<std::endl;
		totalTiming += timing;
		...
		CheckNextPermuteWithOpenCL(arrDest, Max, i+1);
	}

	DestroyMMTimer(wTimerRes, init);
	sprintf(buf, "Total timing: %dms\n", totalTiming);
	std::cout<<buf<<std::endl;
...
void CheckPermuteWithOpenCL(char* arrDest, long long Max)
{
	...
	for(long long i=0; i<Max ;++i)
	{
		for(int j=0;j<NUMBER_OF_ELEMENTS;++j)
		{
			...
			{
				std::cerr<<"Diff check failed at "<<i<<std::endl;
				return;
			}
		}
